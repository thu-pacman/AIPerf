...
def sshKill():
    ...
    for server in SERVER_LIST:
        ...
        os.system("ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 wxp@{} cp /mnt/zoltan/public/dataset/imagenet/aiperf_ctrl/kill.sh /home/wxp".format(server["ip"]))
...
def sshExec(server, trial):
    ...
    bashCmd += (
        "curl --location --request POST 'http://172.23.33.30:9987/api/trial/finish' " +
        "--header 'Content-Type: application/json' --data-raw '{\"trial\":\""
    ...
    os.system("ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 wxp@{} 'cd /mnt/zoltan/public/dataset/imagenet/AIPerf/examples/trials/network_morphism/imagenet/; python3 resource_monitor.py --id {} &'".format(server["ip"], trial["env"]["NNI_EXP_ID"]) )
    os.system("ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 wxp@{} 'mkdir /home/wxp/{} ; cp /mnt/zoltan/public/dataset/imagenet/aiperf_ctrl/tmp.sh /home/wxp/{}'".format(
        server["ip"], 
        trial["env"]["NNI_TRIAL_JOB_ID"],
        trial["env"]["NNI_TRIAL_JOB_ID"]
    ))
    os.system("ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 wxp@{} 'cd /home/wxp/{}; source tmp.sh >stdout.log 2>stderr.log &'".format(server["ip"], trial["env"]["NNI_TRIAL_JOB_ID"]))
    return