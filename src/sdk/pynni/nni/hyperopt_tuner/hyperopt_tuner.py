...
def json2vals(in_x, vals, out_y, name=NodeType.ROOT):
    if isinstance(in_x, dict):
        if NodeType.TYPE in in_x.keys():
            ...
            if _type == 'choice':
                if isinstance(vals,int) or isinstance(vals,str) or isinstance(vals,float):
                    out_y['root[kernel_size]-choice'] = vals
                else:
...
class HyperoptTuner(Tuner):
    ...
    def receive_trial_result(self, parameter_id, parameters, value, **kwargs):
        ...
        # v1.1
        if parameter_id not in self.total_data and parameter_id == 0:
            self.total_data[parameter_id]=parameters
        elif parameter_id not in self.total_data:
            raise RuntimeError('Received parameter_id not in total_data.')
