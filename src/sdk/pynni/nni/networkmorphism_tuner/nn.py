...
import json
from nni.networkmorphism_tuner.graph import Graph, json_to_graph
from nni.networkmorphism_tuner.layers import (StubDense, StubDropout1d, StubAdd,
...
class CnnGenerator(NetworkGenerator):
    ...
    def generate(self, model_len=None, model_width=None):
        ...
        output_node_id = graph.add_layer(
                         self.pooling(kernel_size=3, stride=2), output_node_id)
        ...
        output_node_id00 = graph.add_layer(
            self.batch_norm(
                graph.node_list[output_node_id00].shape[-1]), output_node_id00
        )
    ...
    def pre_generate(self, model_json_dir):
        filename = model_json_dir
        f = open(filename, 'r')
        a = json.load(f)
