        ...
        while not pq.empty() and t > t_min:
            ...
            if ap >= random.uniform(0, 1):
                for temp_graph in transform(elem.graph):
                   ...
                   #     continue
                   # v1.1
                    de_len =len(descriptors)
                    if de_len>=20:
                        if contain(descriptors, temp_graph.extract_descriptor()):
                            continue
                    else:
                        if contain_old(descriptors, temp_graph.extract_descriptor()):
                            continue
...
# v1.1
def contain_old(descriptors, target_descriptor):
...
def job(target_descriptor, descriptor):
    if edit_distance(descriptor, target_descriptor) < 1e-5:
        return 1
    else:
        return 0
